...
int ins_cnt = 0; /// to count number of instructions that each label points to
                 /// in first iteration
...
int main() {
  ...
  while (getline(test, line)) {
    if (line[line.size() - 1] == ':') {
      ...
      if (line.substr(0, 4) == "push" || line.substr(0, 3) == "pop") {
        // cout<<line<<endl;
        if (line[line.size() - 1] == ')' || line.substr(0, 3) == "pop")
          ins_cnt += 3;
        else
          ins_cnt += 2;
      } else
  ...
  while (getline(test1, line)) {
    ...
    if (inst[0][inst[0].size() - 1] != ':') {
      if (inst[0] == "add" || inst[0] == "sub" || inst[0] == "and" ||
          inst[0] == "or" || inst[0] == "nor") {
        ln_cnt++;
        inst[1] = inst[1].substr(0, inst[1].size() - 1);
        inst[2] = inst[2].substr(0, inst[2].size() - 1);

        code = opcode[inst[0]] + registers[inst[2]] + registers[inst[3]] +
               registers[inst[1]] + "0";
        // cout<<code<<endl;
        codefile << code << endl;
      }
        ...
        code = opcode[inst[0]] + "0" + registers[inst[2]] + registers[inst[1]] +
               makehex(inst[3]);
        // cout<<code<<endl;
        codefile << code << endl;
        is_rtype = 0;
      ...
      else if (inst[0] == "addi" || inst[0] == "subi" || inst[0] == "andi" ||
               inst[0] == "ori" || inst[0] == "nori") {
        ln_cnt++;
        inst[1] = inst[1].substr(0, inst[1].size() - 1);
        inst[2] = inst[2].substr(0, inst[2].size() - 1);

        code = opcode[inst[0]] + registers[inst[2]] + registers[inst[1]] +
               makehex(inst[3]);
        // cout<<code<<endl;
        codefile << code << endl;
      }
        ...
        code = opcode[inst[0]] + registers[reg] + registers[inst[1]] +
               makehex(offset);
        // cout<<code<<endl;
        codefile << code << endl;
      }
        ...
        if (inst[1][inst[1].size() - 1] == ')') {
          ...
          code = opcode["lw"] + registers[reg] + registers["$t5"] +
                 makehex(offset);
          codefile << code << endl;
          code =
              opcode["sw"] + registers["$sp"] + registers["$t5"] + makehex("0");
          // cout<<code<<endl;
          codefile << code << endl;
          code = opcode["subi"] + registers["$sp"] + registers["$sp"] +
                 makehex("1");
          // cout<<code<<endl;
          codefile << code << endl;
          ...
          code = opcode["sw"] + registers["$sp"] + registers[inst[1]] +
                 makehex("0");
          codefile << code << endl;
          code = opcode["subi"] + registers["$sp"] + registers["$sp"] +
                 makehex("1");
        ...
        code =
            opcode["addi"] + registers["$sp"] + registers["$sp"] + makehex("1");
        codefile << code << endl;
        code =
            opcode["lw"] + registers["$sp"] + registers[inst[1]] + makehex("0");
        codefile << code << endl;
        code =
            opcode["sw"] + registers["$sp"] + registers["$zero"] + makehex("0");
        codefile << code << endl;
      }
    }
  }
